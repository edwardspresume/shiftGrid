...
<script lang="ts">
	...
	import { CalendarDays, Clock3, MapPin, NotebookPen, Repeat2, Utensils, X } from '@lucide/svelte';
...
<section aria-labelledby="add-shift-title" class="mb-6 rounded-lg border bg-card shadow-sm">
	<form onsubmit={preventPlaceholderSubmit}>
		<header class="flex items-start justify-between gap-4 border-b px-4 py-4">
			<div>
				<h2 id="add-shift-title" class="font-heading text-lg leading-none font-black">Add shift</h2>
				<p class="mt-1 text-sm font-medium text-muted-foreground">{formattedHours} total</p>
			</div>

			<Button variant="ghost" size="icon-sm" aria-label="Close add shift form" onclick={onCancel}>
				<X class="size-4" />
			</Button>
		</header>

		<div class="grid gap-5 p-4 lg:grid-cols-[1fr_18rem]">
		...
		<footer class="flex items-center justify-end gap-2 border-t px-4 py-4">
