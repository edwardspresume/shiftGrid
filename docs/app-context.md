...
- Public registration is disabled; the initial owner account is provisioned by an environment-driven script.
...
- Database and migration code reads server-side connection configuration from environment files.
- Playwright E2E scripts preload the isolated `.env.test` configuration so test data stays out of production.
